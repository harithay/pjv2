...
class DroppableComponent {
	...
	handleMousedownEvent(e, ui) {
		...
		e.detail.nativeUI.helper.css("position", "absolute").css("zIndex", 9999999999999).css("pointerEvents", "none"); // .css("backgroundColor", "red")
	...
	handleMouseupEvent(e, ui) {
		...
		if (type == ModelClassNames.image.type) {
			...
			this.fireCustomEventOnElement(hoveringOverElement, this.createEvent("component-dropped", this.model, { e: e, ui: ui }));
		} else if (type == ModelClassNames.collageContainer.type) {} else if (type == ModelClassNames.shapeCircle.type) {
...
class InputHandler {
	...
	insertText(text) {
		if (this.isCursorWithinAElement()) {
			...
			} else log("Cursor is not within a element.");
		}
	}
...
class SquareTransform {
	main(X_MOUSE_TEST, Y_MOUSE_TEST, POINT_TEST, INIT_THETA_DEG, INIT_X_POS, INIT_Y_POS, INIT_WIDTH, INIT_HEIGHT) {
		var Square = new SquareProfile();
		var test = {};
		test.input = X_MOUSE_TEST + " : " + Y_MOUSE_TEST + " : " + POINT_TEST + " : " + INIT_THETA_DEG + " : " + INIT_X_POS + " : " + INIT_Y_POS + " : " + INIT_WIDTH + " : " + INIT_HEIGHT;
